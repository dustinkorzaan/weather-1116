...
public class GetCurrentAIWeatherV5Handler : IRequestHandler<GetCurrentAIWeatherV5Event, AIWeatherResponse>
{
    ...
    public async Task<AIWeatherResponse> Handle(GetCurrentAIWeatherV5Event request, CancellationToken cancellationToken)
    {
        ...
        // V5 has no local tool loop; the hosted agent resolves tools, so every entry is turn 0.
        const int runLogTurn = 0;
        runLog.AddLog(runLogTurn, $"Start {nameof(GetCurrentAIWeatherV5Handler)}", null);
        ...
        runLog.AddLog(runLogTurn, "Start CreateResponse", null);
        ResponseResult response = await client.CreateResponseAsync(options, cancellationToken);
        runLog.AddLog(runLogTurn, "Finish CreateResponse", response);
        ...
        runLog.AddLog(runLogTurn, $"Finish {nameof(GetCurrentAIWeatherV5Handler)}", null);
