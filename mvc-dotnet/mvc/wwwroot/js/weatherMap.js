window.weatherMap = (function () {
  ...
  const DEFAULT_WEATHER_MODAL_TAB = 'current';
  ...
  function createOneMarker(maps, map, city, appearance, index, LogoPinOverlay, logoUrl) {
    ...
    bindPinHoverCard(
      ...
      city.name,
      function () {
        window.location.assign(weatherModalPath(city.name, city.lat, city.lng, DEFAULT_WEATHER_MODAL_TAB));
      },
      function () {
        removeCity(city.id);
      }
  ...
  function weatherModalPath(name, lat, lng, tab) {
    ...
    params.set('tab', tab || DEFAULT_WEATHER_MODAL_TAB);
  ...
  function init(elementOrId, apiKey, cities) {
    ...
    return loadGoogleMaps(apiKey).then(function (maps) {
      ...
      const entry = {
        ...
        theme: resolvedTheme(),
        getLocationUrl: resolveGetLocationUrl(element),
      };
      const map = createThemedMap(maps, element, appearance, DEFAULT_CENTER, DEFAULT_ZOOM, undefined, function (typeId) {
        entry.mapTypeId = typeId;
      });
