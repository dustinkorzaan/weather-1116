...
8. Set MCP **approval to never** (`require_approval: never`) so Chat3 does not stall.
   Chat3 will auto-approve if a request still appears. V5 (`wx1116-agent-default`) uses
   the same Never setting but has **no** in-app fallback: that round-trip would break
   the V5 one-shot handoff.
