...
| Variable | Used by |
| --- | --- |
| `AZURE_FOUNDRY_PROD_CUS_PROJ_URL` | All chat tabs |
| `AZURE_FOUNDRY_PROD_CUS_KEY` | All chat tabs |
| `AZURE_FOUNDRY_PROD_CUS_MODEL` | Chat1a–Chat2b and Chat4a–Chat4b (not Chat3) |
| `AZURE_FOUNDRY_PROD_CUS_CHAT_AGENT_NAME` | Chat3 only (required). GitHub var / App Service. Independent of V5's `AZURE_FOUNDRY_PROD_CUS_AGENT_NAME`. |
...
`AZURE_FOUNDRY_PROD_CUS_AGENT_NAME` remains the V5 console agent (`wx1116-agent-for-current-weather`,
...
2. **Agents** → `wx1116-agent-for-chat` (must match `AZURE_FOUNDRY_PROD_CUS_CHAT_AGENT_NAME`).
3. Confirm the model is the same deployment as `AZURE_FOUNDRY_PROD_CUS_MODEL`
