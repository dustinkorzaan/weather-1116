...
`ChatMcpToolFactory` (V4 pattern), and `GetCurrentAIWeatherV5Handler` calls a hosted Foundry agent that
...
the V4 handler (`GetCurrentAIWeatherV4Handler`, used by the V4 tab on
`/current-ai-weather`). The V3 handler (`GetCurrentAIWeatherV3Handler`,
used by `/weather`) resolves tools in-process and does not call the MCP
hosts. The V5 handler
...
required for the Chat1b/Chat2b remote-MCP chat tabs. Hello and map chrome do
not need them.
...
- **V5** (`GetCurrentAIWeatherV5Handler`), which calls a hosted Foundry agent
  (`AZURE_FOUNDRY_PROD_CURRENT_WX_AGENT_NAME`) that owns its own instructions,
...
**`/weather`** modal's "current" tab uses **V3** only.

The request path differs by stack:

- **React / Blazor** → `WeatherAPI` (`/AIWeather/CurrentV3`, `/AIWeather/CurrentV4`,
...
ports of Core's `GetLatLongHandler`/`GetLocationHandler` with the same tool names,
...
It then refuses the import, before anything is enqueued, if the export repeats a `GeonameId`, has
100,000 cities or fewer, has under 90% of the rows already in `dbo.Cities`, or has under 1,000 admin1
regions, so a short or broken download never reaches the delete. Only once every file is up and those
checks pass does it enqueue one `ImportCitiesUpsertEvent` per batch file and then one
...
sub-agent, AI Weather V4 and FoundryConsoleV4, and the hosted Foundry toolbox
(Chat3/V5).
...
`GetCurrentAIWeatherV5Handler` needs `AZURE_FOUNDRY_PROD_CURRENT_WX_AGENT_NAME`
...
| Variable | Required | Purpose |
| --- | --- | --- |
| `AZURE_FOUNDRY_PROD_CHAT_AGENT_NAME` | Yes (Chat3) | Chat3 only. GitHub variable `AZURE_FOUNDRY_PROD_CHAT_AGENT_NAME` (API/MVC App Service). Independent of V5's `AZURE_FOUNDRY_PROD_CURRENT_WX_AGENT_NAME`. MCP tools on this agent must use `require_approval: never`; Chat3 does not round-trip approvals. |
