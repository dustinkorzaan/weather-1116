...
| Foundry Console V1–V4 | [`FoundryConsoleV1…`](../FoundryConsoleV1ModelDirectLegacyCognitiveServicesEndpoint) … [`V4`](../FoundryConsoleV4MCP) | Local learning demos for Foundry / agent patterns (in `Weather.sln` as `WeatherFoundryConsole.V1`–`V4MCP`; built in CI) |
...
- `Core.csproj`, `WeatherAPI.csproj`, `WeatherBlazor.csproj`, `WeatherMVC.csproj`,
  ...
  and the four `FoundryConsole*` `WeatherFoundryConsole.csproj` demos via `dotnet build`.
