...
upserts 1,000 cities at a time keyed on `GeonameId` (one lookup and one save per chunk), then rows GeoNames no longer lists are bulk-deleted. Until that job has run once in an
