...
- **V5** (`GetCurrentAIWeatherV5Handler`), which calls a hosted Foundry agent
  (`AZURE_FOUNDRY_PROD_AGENT_NAME`) that owns its own instructions,
...
`GetCurrentAIWeatherV5Handler` needs `AZURE_FOUNDRY_PROD_AGENT_NAME`
...
2. **Agents** → `wx1116-agent-for-current-weather` (or `AZURE_FOUNDRY_PROD_AGENT_NAME`).
...
| Variable | Required | Purpose |
| --- | --- | --- |
| `AZURE_FOUNDRY_PROD_CHAT_AGENT_NAME` | Yes (Chat3) | Chat3 only. GitHub variable `AZURE_FOUNDRY_PROD_CHAT_AGENT_NAME` (API/MVC App Service). Independent of V5's `AZURE_FOUNDRY_PROD_AGENT_NAME`. MCP tools on this agent must use `require_approval: never`; Chat3 does not round-trip approvals. |
