...
import { useLazyGetCurrentAIWeatherV4Query } from '../../services/weatherApi';
import {
  formatLatLong,
  formatRunLogMs,
...
/**
 * V4 tab: remote MCP tools, model host runs the tool loop (like Foundry Console
 * V4). Copied (not shared) from the V3/V5 tabs — this repo duplicates each AI
 * weather variant rather than parameterizing one generic component.
 */
function CurrentAIWeatherV4Tab() {
  const [location, setLocation] = useState('Nashville, TN');
  const [trigger, { data, isFetching, isError, error }] = useLazyGetCurrentAIWeatherV4Query();
...
export default CurrentAIWeatherV4Tab;
