...
function DailyHistoryTab({ lat, lng }) {
  ...
  const rows = (daily?.time ?? [])
    .map((time, index) => ({
      ...
      windDirection: daily.windDirectionToDegrees[index],
    }))
    .reverse();
  ...
  return (
    <section aria-labelledby="daily-history-heading">
      <div className="mb-3 flex items-center justify-between gap-2">
        <h2 id="daily-history-heading" className="text-xl font-semibold">
          Daily History
        </h2>
        <Button
          type="button"
      ...
      {isError && <p className="text-destructive">Unable to load daily history.</p>}
...
export default DailyHistoryTab;
