...
public class Chat4aControllerTests(ChatApiWebApplicationFactory factory) : IClassFixture<ChatApiWebApplicationFactory>
{
    ...
    public async Task PostMessage_ReturnsCamelCaseSseEvents()
    {
        ...
        Assert.Collection(
            ...
            sessionEvent =>
            {
                ...
                Assert.Equal("Chat4a:test-session", sessionEvent.GetProperty("sessionId").GetString());
            },
            tokenEvent =>
            {
                Assert.Equal("token", tokenEvent.GetProperty("type").GetString());
                Assert.Equal("Hello from Chat4a", tokenEvent.GetProperty("text").GetString());
            },
...
internal sealed class StubChat4aClientService : IChatClientService
{
    public async IAsyncEnumerable<ChatStreamEvent> SendMessageAsync(
        ChatSendMessageRequest request,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        yield return ChatStreamEvent.Session("Chat4a:test-session");
        yield return ChatStreamEvent.Token("Hello from Chat4a");
