...
echo "$PAYLOAD" | jq -e '.connection_body.category == "RemoteTool"' >/dev/null \
  || fail "toolbox connection should be RemoteTool"
echo "$PAYLOAD" | jq -e '.connection_body.authType == "ProjectManagedIdentity"' >/dev/null \
  || fail "toolbox connection should use project managed identity"
echo "$PAYLOAD" | jq -e '.connection_body.target == $consumer' --arg consumer "$CONSUMER_URL" >/dev/null \
