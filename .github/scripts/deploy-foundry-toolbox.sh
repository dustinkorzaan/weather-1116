...
#
# The toolbox wraps three of the IaC-provisioned MCP RemoteTool connections
# (MyMcpSrvFuncApp, MyMcpSrvPython, MyMcpSrvNode). MyMcpSrvAppService is left out:
# it serves the user/pin tools, which the hosted geo + weather agents must not get. Agents attach the
...
VERSION_BODY=$(jq -n \
  --arg description "Geo + NonAI Weather toolbox (func-app/python geo + node weather)" \
